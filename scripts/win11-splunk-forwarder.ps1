# Provisioning Windows 11 (workstation) - Splunk Universal Forwarder
# SOC Blue Team Lab. Only runs when the siem VM is running Splunk
# (ENABLE_SPLUNK=1). Requires win11-baseline.ps1 to have run first.
...
$LogFile = "$LogDir\win11-splunk-forwarder.log"
...
log "=========================================================="
log "  Win11 Splunk Universal Forwarder setup started  |  SIEM: $SIEM_IP"
...
# System - PowerShell/Operational (4103/4104, win11-baseline.ps1), Sysmon/
# Operational (win11-baseline.ps1), Windows Defender/Operational
# (win11-defender-telemetry.ps1), and DNS-Client/Operational
# (win11-dns-telemetry.ps1 - no DNS-Server channel here, win11 isn't a DNS
# server) all need their own inputs.conf stanza - MiniLab-1f7, MiniLab-1d0,
# MiniLab-qju. PowerShell Transcription (win11-powershell-transcription.ps1)
# and Windows Firewall logging (win11-firewall-logging.ps1) are both
# file-based, not event channels, so they're monitor:// stanzas instead of
...
"@ | Out-File -FilePath "$LogDir\win11-splunk-uf-credentials.txt" -Encoding ascii
ok "Credentials saved to logs/win11-splunk-uf-credentials.txt"

# -- 6. Summary ----------------------------------------------------------------
log "=========================================================="
log "  Win11 Splunk Universal Forwarder setup COMPLETED"
log "  Forwarding to: ${SIEM_IP}:9997"
log "  Full log:      logs/win11-splunk-forwarder.log"
log "=========================================================="
